    ...
    async changeViewMyLib(newIndex) {
        try {
            if (newIndex !== this.selectedIndexMyLib) {
                ...
                if (newIndex == 0) {
                    Utils.player.getState().then((playing) => {
                        let isPlay = Utils.queueManager.currentObject != null && "pl_" + Utils.libManager.userInfo.likedSongsPlId == Utils.queueManager.currentObject.id && playing
